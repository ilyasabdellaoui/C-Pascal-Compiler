...
typedef enum {
    ...
    PRN, INN, INT, LDI, LDA, LDV, STO, BRN, BZE, HLT, LABEL
} INSTS;
...
void ECRIRE1(INSTS MNEM) {
    if (SP == MEM_SIZE - 1) { // Check stack overflow
...
void IDENTIFY_INST(char *line) {
    // Assuming P-code instructions are separated by spaces
